...
fun <S> movePlayer(player: Rect2, terrainSize: Double, terrain: List<List<S>>,
                   accessFn: (S) -> (Boolean),
                   calcNewAmountFn: (Double, Double, Rect2, Double) -> Pair<Rect2, Double>,
                   amount: Int): Double {
    val utils = Utils(50)
    var curAmount = 0.0
    while (curAmount < amount) {
        ...
        println("Mid: ${player.midpoint()}")
        println("Pi: $pi")

        val (newPlayerRect, newAmount) = calcNewAmountFn(curAmount,
                amount.toDouble(), player, terrainSize)
        val indices = utils.rectToIndex(newPlayerRect)

        println(newPlayerRect)
        println(indices)
...
val upFn: (Double, Double, Rect2, Double) -> Pair<Rect2, Double> =
        { curAmount: Double, maxAmount: Double, player: Rect2, tileSize: Double ->
...
val downFn: (Double, Double, Rect2, Double) -> Pair<Rect2, Double> =
        { curAmount: Double, maxAmount: Double, player: Rect2, tileSize: Double ->
            val amountFromStart = tileSize - (player.ly - curAmount) % tileSize
            debugln("(${player.ly} - $curAmount) % $tileSize")
            val newAmount =
                    min(maxAmount.toDouble(), curAmount + amountFromStart)
            val newPlayerRect = player.minus(Vec2(0.0, newAmount))
            debugln("$amountFromStart, $newAmount")
            Pair(newPlayerRect, newAmount)
        }

val rightFn: (Double, Double, Rect2, Double) -> Pair<Rect2, Double> =
        { curAmount: Double, maxAmount: Double, player: Rect2, tileSize: Double ->
...
val leftFn: (Double, Double, Rect2, Double) -> Pair<Rect2, Double> =
        { curAmount: Double, maxAmount: Double, player: Rect2, tileSize: Double ->
